...
def liquidity_arrives(role: str) -> bool:
    return role in {"ACTIVATION", "OVERLAP BUILD", "HANDOFF"}
...
def prior_impulse_up(
    ...
    atr_prev: float,
    impulse_atr_mult: float = 0.80,
) -> bool:
    return (
        atr_prev == atr_prev
        and swing_low_prev == swing_low_prev
        and (close_prev - swing_low_prev) >= impulse_atr_mult * atr_prev
        and close_prev > close_lookback
    )
...
def setup_ready(
    ...
    return boundary_bar and prior_trend and prior_impulse and role != "INTERNAL BOUNDARY"
